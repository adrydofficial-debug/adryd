// src/features/payment/screens/CardPaymentScreen.tsx
// Updated: Uses PayFast form POST model for secure payments
...
type CardPaymentRouteProp = RouteProp<AppStackParamList, 'CardPaymentScreen'>;
...
const CardPaymentScreen: React.FC = () => {
  ...
  const handleInitiatePayment = useCallback(async () => {
    setLoading(true);
    try {
      // Call our backend API to get checkoutUrl and formData
  ...
  }, [campaignId, amount, customerEmail, customerPhone]);

  // Check payment status after WebView callback
    ...
    try {
      ...
      if (statusResponse.status === 'completed') {
        ...
        navigation.replace('PaymentResultScreen', {
          ...
          transactionId,
          amount,
          errorMessage: statusResponse.message || 'Payment was declined or failed',
          campaignId: campaignId,
        });
      }
    } catch (error) {
  ...
  const handleNavigationChange = useCallback(
    (navState: any) => {
      ...
      // Check for success callback URL from our backend
      // PayFast typically returns to return_url on success
      if (url.includes('/callback/success') || 
        ...
        // Verify status with backend before showing success
        checkPaymentStatus();
        navigation.replace('PaymentResultScreen', {
          status: 'success',
      ...
      if (url.includes('/callback/failure') ||
        ...
        const errorMessage = params.err_msg || params.error_message || 'Payment was declined or failed';
      ...
      // Check for cancel callback URL
      if (url.includes('/callback/cancel') ||
          url.includes('/payment/cancel') || 
          url.includes('status=cancel') ||
          url.includes('/payments/callback/cancel')) {
        setPaymentHtml(null);
    ...
    <View style={styles.container}>
      ...
      <View style={styles.content}>
        ...
        <View style={styles.infoCard}>
          <Text style={styles.infoTitle}>Secure Card Payment</Text>
